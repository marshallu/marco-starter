...
function marco_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'marco' ),
		)
	);
...
function marco_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'marco' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'marco' ),
...
function marco_scripts() {
	wp_enqueue_style( 'marco-style', marco_vite( '/css/app.css' ) ); // phpcs:ignore

	wp_enqueue_script( 'marco-scripts', marco_vite( '/js/app.js' ) ); // phpcs:ignore
 ...
 * @return string
 */
function marco_vite( $filename ) {
	$filename = 'source' . $filename;

	$mix_data   = file_get_contents( get_template_directory() . '/public/build/manifest.json' );
	$files_list = json_decode( $mix_data, true );

	if ( array_key_exists( $filename, $files_list ) ) {
		return esc_url( get_theme_file_uri( '/public/build/' . $files_list[ $filename ]['file'] ) );
	}
}
